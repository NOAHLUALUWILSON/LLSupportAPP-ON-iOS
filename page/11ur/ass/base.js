$(document).ready(function()
    {
        $.facebox.settings.closeImage = 'ass/closelabel.png'
        $.facebox.settings.loadingImage = 'ass/loading.gif'
        var carddealer = {
            "init": function() {
                $("#total").attr("value", "0");
                $("#heart").attr("value", "0");
            ...
            "deal": function(big)
            {
                ...
                var lastFilter = function(card) {
                    ...
                    if(card.rarity == 'R') {
                        if(card.id < 700) return true;
                        return false;
                    }
                    if(740 <= card.id && card.id <= 764) return false;
                    var banlist = [711, 728, 774, 784, 810, 817, 835];
                    for(id in banlist) {
                        if(id == card.id) return false;
                    }
                    if(card.id == 551) return true;
                    if(558 <= card.id && card.id <= 843) return true;
                    return false;
                }
                // SR
