...
function CoverageOfSingleNote(s, _offset)
{
    ...
    for (var i = 0; i < Map_size; i++)
    {
        ...
        if (Map[i].effect == 3)
        {
            ...
            TempCoverage[i] = TempCoverage[i] / (4 * (a2 - a1));
        }
        else
        {
            var a1 = Math.round(Map[i].timing_sec * 1000 - _offset * Offset[s - 1] - 1000 * Good[s - 1]);
    ...
    */
    ...
    for (var i = 0; i < Map_size; i++)
    {
        ...
        if (Map[i].effect == 3)
        {
            ...
            TempCoverage[i] = (TempTempCoverage / (2 * (a2 - a1)) + TempCoverage[i]) / 2;
        }
    }
    return TempCoverage;
}
...
$(function ()
{
    $('#container').highcharts({
        ...
        tooltip:
        {
            ...
            positioner: function()
            {
                return {x: 425, y: containerheight}
            },
            formatter: function()
            {
...
function CoverageCalculator()
{
    ...
    if (_offset < -50 || _offset > 50)
    {
        ...
        //获取BPM并处理
    bpm = Number(songs[document.getElementById('songchoice').value].bpm);
    if (bpm == 0 || !isNumber(bpm))
    {
        Crotchet = NaN;
        bpm = 60;
    }
    ...
    var maps = songs[document.getElementById('songchoice').value][document.getElementById('diffchoice').value].liveid;
        //+判断库中该谱面文件是否存在
    if (Number(maps) >= 559 && Number(maps) <= 560)
    {
